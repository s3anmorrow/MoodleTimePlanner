...
(function ($) {
    ...
    var methods = {
        ...
        init: function (conf) {
            ...
            for (var i in formControls) {
                formControls[i] = ':input[id="' + formControls[i] + '"]:not([disabled])';
            }

            formControls = formControls.join(',');

            // storing configuration data in the DOM element as data so that other methods in this plugin can have access to it
            this.data({ "conf": config, "controls": formControls });

            return this.each(function () {
                jQuery(formControls, this).each(function () {
                    var t = jQuery(this);
                    var isOptional = false;
                    var fieldName = t.attr('id');
                    ...
                    // this is here to avoid setting up all below twice on the same controls
        ...
        isValid: function () {
            // retrieve data from the element that was determined in the init method (config object)
            var config = this.data("conf");
            // assume valid by default
            var valid = true;
            // how many invalid images are in the targeted parent element?
            try {
                var numOfInvalids = jQuery(this).find('img[src="' + config.invalidIco + '"]').length;
                if (numOfInvalids > 0) {
                    valid = false;
                }
            } catch (err) {
        ...
        validateNow: function () {
            // retrieve data from the element that was determined in the init method (formControls array)
            var formControls = this.data("controls");
            jQuery(formControls, this).each(function () {
                var t = jQuery(this);
                //window.alert("validating: " + t.attr('id'));
                // trigger the keyup event for all formControls that are being validated (since it is already wired up)
                t.trigger("keyup");
            });
        }
    ...
    jQuery.fn.liveValidation = function (method) {
        // Method calling logic
        if (methods[method]) {
            return methods[method].apply(this, Array.prototype.slice.call(arguments, 1));
        } else if (typeof method === 'object' || !method) {
            // apply allows you to call a function and pass on all the arguments via tha argument parameter
            return methods.init.apply(this, arguments);
        } else {
            $.error('Method ' + method + ' does not exist on jQuery.liveValidation');
        }
    };
