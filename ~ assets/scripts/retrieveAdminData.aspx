...
<script runat="server">
    ...
    Sub Page_Load
        ...
        if (Session("login")) then
			...
			'Dim DBCommand = New OleDbCommand("SELECT deadlines.*, login.fullName FROM deadlines INNER JOIN login ON deadlines.author=login.login WHERE author='" & Request.Item("author") & "' OR author='general' ORDER BY id DESC", DBConnection)
			Dim DBCommand = New OleDbCommand("SELECT deadlines.*, login.fullName FROM deadlines INNER JOIN login ON deadlines.author=login.username WHERE author='" & Session("user") & "' OR author='general' ORDER BY id DESC", DBConnection)
